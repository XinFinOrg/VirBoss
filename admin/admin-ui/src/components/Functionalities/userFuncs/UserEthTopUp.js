...
class UserEthTopUp extends Component {
  constructor(props) {
    super(props);
    this.state = { userEmail: "", ethAmount: null };
  ...
  handleSubmit() {
    ...
    const ethAmountValid = validate("integer", this.state.ethAmount);
    if (!emailValid) {
      return showNotification("danger", "XDC Top-up", "Invalid email");
    } else if (!ethAmountValid) {
      return showNotification("danger", "XDC Top-up", "Invalid token amount");
    }
    if (parseInt(this.state.ethAmount, 10) <= 0) {
      return showNotification("danger", "XDC Top-up", "Invalid token amount");
    }
    // data is valid
    axios
      .post("/api/userEthTopUp", {
      ...
      .then(async resp => {
        if (resp.data.status === true) {
          ...
          this.setState({userEmail:"",ethAmount:""});
        } else {
          // show error
          this.setState({
            showError: true,
