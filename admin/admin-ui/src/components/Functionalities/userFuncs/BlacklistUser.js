...
class BlacklistUser extends Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit() {
    ...
    if (!emailValid) {
      return showNotification("danger", "Blacklist User", "Invalid email");
    }
    // data is valid
    axios
      .post("/api/blacklistUser", {
      ...
      .then(resp => {
        if (resp.data.status === true) {
          ...
          this.setState({
            showSuccess: true,
            successMsg: `user ${this.state.userEmail} has been blacklisted!`,
            userEmail: ""
          });
        } else {
          // show error
